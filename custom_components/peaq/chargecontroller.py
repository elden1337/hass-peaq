...
from custom_components.peaq.constants import CHARGERCONTROLLER
...
class ChargeController():
    ...
    def _let_charge(self) -> str:
            if self._hub.chargerobject.value.lower() in self._hub.chargertypedata.chargerstates["idle"] :
                return constants.CHARGECONTROLLER.Idle
            elif self._hub.chargerobject.value.lower() not in self._hub.chargertypedata.chargerstates["idle"] and self._hub.charger_done.value == True:
                return constants.CHARGECONTROLLER.Done
            elif datetime.now().hour in self._hub.nonhours:
                return constants.CHARGECONTROLLER.Stop
            elif self._hub.chargerobject.value.lower() in self._hub.chargertypedata.chargerstates["connected"]:
                if self.below_startthreshold and self._hub.totalhourlyenergy.value > 0:
                    return constants.CHARGECONTROLLER.Start
                else:
                    return constants.CHARGECONTROLLER.Stop
            elif self._hub.chargerobject.value.lower()  in self._hub.chargertypedata.chargerstates["charging"]:
