...
class PeaqMoneySensor(SensorBase, RestoreEntity):
    ...
    async def async_update(self) -> None:
        ...
        if ret is not None:
            ...
            self._nonhours = await self.async_set_non_hours_display(
                ret.get("non_hours"), ret.get("prices_tomorrow")
            )
            self._dynamic_caution_hours = await self.async_set_caution_hours_display(
            ...
            self._avg_cost = await self.async_currency_translation(
                value=ret.get("avg_kwh_price"),
                currency=ret.get("currency"),
                use_cent=ret.get("use_cent"),
            )

            self._average_nordpool = await self.async_currency_translation(
                value=ret.get("average_weekly"),
                currency=ret.get("currency"),
                use_cent=ret.get("use_cent"),
            )
            self._average_data_current_month = await self.async_currency_translation(
