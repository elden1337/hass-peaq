...
from custom_components.peaqev.peaqservice.chargecontroller.charger.const import LOOP_WAIT, LOOP_RELEASE_TIMER
...
class ChargerHelpers:
    ...
    def _currents_match(self) -> bool:
        return self.charger.controller.hub.sensors.chargerobject_switch.current == self.charger.controller.hub.threshold.allowedcurrent  # todo: composition

    def _too_late_to_increase(self) -> bool:
        return datetime.now().minute >= 55 and self.charger.controller.hub.threshold.allowedcurrent > self.charger.controller.hub.sensors.chargerobject_switch.current  # todo: composition
