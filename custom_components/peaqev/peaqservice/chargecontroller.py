...
class ChargeController:
    ...
    def _let_charge(self):
        ...
        if charger_state in self._hub.chargertypedata.charger.chargerstates[CHARGECONTROLLER.Idle]:
            update_timer = True
            ret = CHARGECONTROLLER.Idle
        elif charger_state in self._hub.chargertypedata.charger.chargerstates[CHARGECONTROLLER.Connected] and self._hub.charger_enabled.value is False:
            update_timer = True
            ret = CHARGECONTROLLER.Connected
        elif charger_state not in self._hub.chargertypedata.charger.chargerstates[CHARGECONTROLLER.Idle] and self._hub.charger_done.value is True:
            ret = CHARGECONTROLLER.Done
        elif datetime.now().hour in self._hub.nonhours:
            update_timer = True
            ret = CHARGECONTROLLER.Stop
        elif charger_state in self._hub.chargertypedata.charger.chargerstates[CHARGECONTROLLER.Connected]:
            if self._hub.carpowersensor.value < 1 and time.time() - self.latest_charger_start > DONETIMEOUT:
                ...
                if self.below_startthreshold and self._hub.totalhourlyenergy.value > 0:
                    ...
                    update_timer = True
                    ret = CHARGECONTROLLER.Stop
        elif charger_state in self._hub.chargertypedata.charger.chargerstates[CHARGECONTROLLER.Charging]:
            update_timer = True
