...
#     @staticmethod
#     def _call_func(func: Callable, command: Command):
...
#     @staticmethod
#     async def async_call_func(func: Callable, command: Command):
#         if command.argument is not None:
#             if isinstance(command.argument, dict):
#                 try:
...
class Observer:
    ...
    def __init__(self, hub):
        self.model = ObserverModel()
        self.hub = hub
        self.hub.state_machine.bus.async_listen(
            EVENT_STATE_CHANGED, self.async_dispatch
        )
            ...
            if command.expiration > time.time():
                ...
                for func in self.model.subscribers[command.command]:
                    if await async_iscoroutine(func):
                        await self.async_call_func(func=func.function, command=command),
                    else:
