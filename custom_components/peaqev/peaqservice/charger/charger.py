...
class Charger:
    ...
    async def charge(self):
        ...
        if self._hub.sensors.charger_enabled.value is True:
            ...
            if self._hub.chargecontroller.status is CHARGERSTATES.Start.name:
                ...
                if (self._chargertype_charger_is_on or self._hub.sensors.carpowersensor.value > 0) is True and self._charger_stopped is False:
    ...
    async def _updatemaxcurrent(self):
        ...
        if await self._hass.async_add_executor_job(self._wait_turn_on):
            ...
            while self._hub.sensors.chargerobject_switch.value is True and self._charger_running is True:
    ...
    def _wait_update_current(self) -> bool:
        self._hub.sensors.chargerobject_switch.updatecurrent()
        while (self._hub.sensors.chargerobject_switch.current == self._hub.threshold.allowedcurrent
               or (datetime.now().minute >= 55
                   and self._hub.threshold.allowedcurrent > self._hub.sensors.chargerobject_switch.current)) \
