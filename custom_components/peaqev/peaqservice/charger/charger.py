...
class Charger:
    ...
    async def charge(self):
        ...
        if self._hub.sensors.charger_enabled.value and not self._hub.sensors.power.killswitch.is_dead:
            ...
            if self._charger_is_active and self.params.running:
    ...
    async def _start_charger(self):
        if time.time() - self.params.latest_charger_call > CALL_WAIT_TIMER and not self._hub.sensors.power.killswitch.is_dead:
            await self._update_charger_state_internal(ChargerStates.Start)
    ...
    async def _updatemaxcurrent(self):
        ...
        if await self._hass.async_add_executor_job(self.helpers.wait_turn_on):
            ...
            while self._hub.sensors.chargerobject_switch.value is True and self.params.running is True:
                if await self._hass.async_add_executor_job(self.helpers.wait_update_current):
                    ...
                    if not self.params.disable_current_updates and self._hub.power_canary.allow_adjustment(new_amps=serviceparams[calls[PARAMS][CURRENT]]):
