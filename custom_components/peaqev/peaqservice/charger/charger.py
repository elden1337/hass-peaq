...
class Charger:
    ...
    def session_active(self) -> bool:
        return self._params._session_active

    @session_active.setter
    def session_active(self, val):
        self._params._session_active = val
    ...
    async def charge(self):
        ...
        if self._hub.sensors.charger_enabled.value is True:
            ...
            if self._hub.chargecontroller.status is CHARGERSTATES.Start.name:
                if self._chargertype_charger_is_on is False and self._params._running is False:
                    await self._start_charger()
            elif self._hub.chargecontroller.status in [CHARGERSTATES.Stop.name, CHARGERSTATES.Idle.name]:
                if (self._chargertype_charger_is_on or self._hub.sensors.carpowersensor.value > 0) is True and self._params._stopped is False:
            ...
            if self._chargertype_charger_is_on is True and self._params._stopped is False:
                await self._terminate_charger()
            return
    ...
    async def _call_charger(self, command: str):
        ...
        _LOGGER.debug(msg)
        self._params._latest_charger_call = time.time()
    ...
    async def _updatemaxcurrent(self):
        ...
        if await self._hass.async_add_executor_job(self.helpers.wait_turn_on):
            ...
            while self._hub.sensors.chargerobject_switch.value is True and self._params._running is True:
                if await self._hass.async_add_executor_job(self.helpers.wait_update_current):
                    ...
                    if not self._params._disable_current_updates:
