    ...
    if not hub.options.peaqev_lite:
        ...
        if any(
            ...
            if hub.chargertype.type is not ChargerType.NoCharger:
                ret.append(PeaqIntegrationSensor(hub,f"sensor.{DOMAIN}_{hub.sensors.power.house.id}",f"{ex.nametoid(CONSUMPTION_INTEGRAL_NAME)}",config.entry_id))
            ret.append(PeaqIntegrationSensor(hub,hub.sensors.power.total.entity,f"{ex.nametoid(CONSUMPTION_TOTAL_NAME)}",config.entry_id))
        else:
            ret.append(PeaqIntegrationSensor(hub,hub.sensors.power.house.entity,f"{ex.nametoid(CONSUMPTION_INTEGRAL_NAME)}",config.entry_id))
            ret.append(PeaqIntegrationSensor(hub,f"sensor.{DOMAIN}_{hub.sensors.power.total.id}",f"{ex.nametoid(CONSUMPTION_TOTAL_NAME)}",config.entry_id))
